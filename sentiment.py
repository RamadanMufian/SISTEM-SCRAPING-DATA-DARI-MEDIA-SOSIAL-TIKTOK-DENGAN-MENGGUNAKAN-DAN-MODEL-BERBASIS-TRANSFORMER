...
print("="*60)
print("LOADING OPTIMIZED SENTIMENT SYSTEM (V6.1 - TIME SERIES)")
print("="*60)
print(f"Advanced Model: {MODEL_NAME}")
print(f"Simple Model: {SIMPLE_MODEL_NAME}")
print(f"Device: {DEVICE}")
print(f"Negative Multiplier: {NEGATIVE_MULTIPLIER}")
print(f"Positive Boost: {POSITIVE_BOOST}")
print(f"Positive Bias: {POSITIVE_BIAS}")
...
try:
    ...
    print(f"✅ Advanced model loaded in {load_time:.2f}s")
except Exception as e:
    print(f"⚠️ Error loading IndoBERT: {e}")
    MODEL_NAME = "w11wo/indonesian-roberta-base-sentiment-classifier"
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
    model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME)
    model.eval()
    model = model.to(DEVICE)
    print(f"✅ Using fallback advanced model: {MODEL_NAME}")
...
try:
    ...
    print(f"✅ Simple model loaded: {SIMPLE_MODEL_NAME}")
except Exception as e:
    print(f"⚠️ Error loading simple model: {e}")
...
def format_trend_chart(days=30):
    ...
    # Untuk chart, kita balik urutannya (terbaru di kanan)
    dates = dates[::-1]
...
class RandomForestValidator:
    ...
    def _load_model(self):
        if os.path.exists(RF_MODEL_PATH):
            try:
                ...
                print(f"✅ Random Forest validator loaded")
            except Exception as e:
    ...
    def save_model(self):
        ...
        print(f"✅ Random Forest validator saved")
    ...
    def train(self, X_train, y_train):
        print("\n📊 Training Random Forest Validator...")
        ...
        print(f"✅ Random Forest trained! Accuracy: {train_acc:.4f}")
        self.save_model()
        return train_acc
...
def apply_hard_rules(text):
    ...
    for pattern, label in COMMON_PATTERNS.items():
        if pattern in text_lower:
            print(f"   📋 Hard rule matched: '{pattern}' -> {label}")
            return label, 85.0
...
def calibrate_confidence(label, confidence):
    ...
    if label == "Negatif" and confidence < 70:
        return "Netral", 60
    if label == "Positif" and confidence < 65:
        return "Netral", 60
...
def advanced_transformer_predict(text):
    try:
        ...
        print(f"⚠️ Advanced transformer error: {e}")
...
def simple_sentiment(text):
    ...
    try:
        ...
        print(f"⚠️ Simple model error: {e}")
        return "Netral", 50.0
...
def smart_decision_engine(text, v4_label, v4_conf, simple_label, simple_conf):
    ...
    print(f"   🤔 DECISION: V6={v4_label}({v4_conf:.0f}) vs Simple={simple_label}({simple_conf:.0f})")
    
    if v4_conf < 60 and simple_conf < 60:
        print(f"   ⚖️ BOTH LOW CONFIDENCE: Netral")
        return "Netral", 60
    
    if abs(v4_conf - simple_conf) > 20:
        if v4_conf > simple_conf:
            print(f"   🎯 V6 lebih yakin: {v4_label}")
            return v4_label, v4_conf
        else:
            print(f"   🎯 SIMPLE lebih yakin: {simple_label}")
            return simple_label, simple_conf
    
    if v4_label == simple_label:
        final_conf = max(v4_conf, simple_conf)
        print(f"   ✅ AGREEMENT: {v4_label}")
        return v4_label, final_conf
    
    if v4_conf > 75:
        print(f"   🔥 V6 high confidence: {v4_label}")
        return v4_label, v4_conf
    
    if simple_conf > 75:
        print(f"   🔥 SIMPLE high confidence: {simple_label}")
        return simple_label, simple_conf
    
    print(f"   ⚖️ CONFLICT DEFAULT: Netral")
    return "Netral", 65
...
def analyze_sentiment(text, return_confidence=False, return_details=False, true_label=None, save_to_history=True):
    ...
    print(f"   🧠 V6.0: {v4_label} ({v4_conf:.1f}%)")
    
    # Simple Model
    simple_label, simple_conf = simple_sentiment(text)
    print(f"   🤖 SIMPLE: {simple_label} ({simple_conf:.1f}%)")
    ...
    print(f"   🎯 FINAL: {final_label} ({final_confidence:.1f}%)")
    ...
    if true_label and true_label != final_label:
        ...
        try:
            ...
            if training_data_count > 200:
                print(f"   🔄 Auto-retraining RF with {training_data_count} samples...")
...
def train_rf_from_feedback():
    ...
    if len(training_data) < 50:
        print(f"⚠️ Need at least 50 samples for training. Currently: {len(training_data)}")
        return None
...
def analyze_batch(texts, batch_size=16, save_to_history=True):
    results = []
    total = len(texts)
    print(f"📊 Menganalisis {total} teks dalam batch...")
    start_time = time.time()
    ...
    print(f"✅ Batch analysis selesai dalam {elapsed:.2f}s")
    return results
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("🧪 TESTING OPTIMIZED MODEL (V6.1 - TIME SERIES)")
    print("="*60)
    ...
    for text, expected in test_cases:
        ...
        print(f"\n{status} '{text}'")
        print(f"   Result: {sentiment} ({conf:.1f}%) | Expected: {expected}")
    ...
    # 🔥 TEST TIME SERIES
    print("\n" + "="*60)
    print("📊 TIME SERIES DATA TEST")
    print("="*60)
    
    history_stats = get_history_stats()
    print(f"📈 History stats: {history_stats['total']} total analisis")
    print(f"   Positif: {history_stats['positif']}")
    print(f"   Negatif: {history_stats['negatif']}")
    print(f"   Netral: {history_stats['netral']}")
    
    trend_data = get_trend_data(days=7)
    print(f"\n📊 Trend data (7 hari terakhir):")
    for i, date in enumerate(trend_data["labels"][:7]):
        print(f"   {date}: Positif={trend_data['positif'][i]}, Negatif={trend_data['negatif'][i]}, Netral={trend_data['netral'][i]}")
